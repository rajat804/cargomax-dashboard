...
export const createGoodsArrival = async (data) => {
  console.log('Creating goods arrival with data:', data);
  const response = await api.post('/goods-arrival', data);
  console.log('Create response:', response.data);
  return response.data;
};

export const getGoodsArrivals = async (params) => {
  const response = await api.get('/goods-arrival', { params });
  return response.data;
};
...
export const exportGoodsArrivals = async (params) => {
  const response = await api.get('/goods-arrival/export', { params });
  return response.data;
};

export const getGoodsArrivalStats = async () => {
  const response = await api.get('/goods-arrival/stats');
  return response.data;
};

export const cancelGoodsArrival = async (id) => {
  const response = await api.patch(`/goods-arrival/${id}/cancel`);
  return response.data;
};

export const restoreGoodsArrival = async (id) => {
  const response = await api.patch(`/goods-arrival/${id}/restore`);
  return response.data;
};
 ...
 * @param {Object} data - Purchase bill data
 * @returns {Promise} - API response
 */
export const createPurchaseBill = async (data) => {
  console.log('📤 Creating purchase bill with data:', data);
  const response = await api.post('/purchase-bills', data);
  console.log('📥 Create purchase bill response:', response.data);
  return response.data;
};

/**
 * Get pending purchase orders with filters
 * @param {Object} params - { branch, vendor, asOnDate }
 * @returns {Promise} - List of pending POs
 */
export const getPendingPurchaseOrders = async (params) => {
  console.log('📤 Fetching pending POs with params:', params);
  const response = await api.get('/purchase-bills/pending-pos', { params });
  console.log('📥 Pending POs response:', response.data);
  return response.data;
};

/**
 * Search vendor bill receipts
 * @param {Object} params - { fromDate, toDate, filterOn, invoiceCategory }
 * @returns {Promise} - List of receipts
 */
export const getVendorBillReceipts = async (params) => {
  console.log('📤 Searching vendor bill receipts with params:', params);
  const response = await api.get('/purchase-bills/receipts', { params });
  console.log('📥 Search receipts response:', response.data);
  return response.data;
};


/// ==================== GET PURCHASE BILL BY ID ====================
export const getPurchaseBillById = async (receiptId) => {
  const response = await api.get(`/purchase-bills/${receiptId}`);
  return response.data;
};

export const updatePurchaseBill = async (receiptId, data) => {
  const response = await api.put(`/purchase-bills/${receiptId}`, data);
  return response.data;
};

export const deletePurchaseBill = async (receiptId) => {
  const response = await api.delete(`/purchase-bills/${receiptId}`);
  return response.data;
};


// ==================== STATIONERY STOCK REGISTER APIs ====================

/**
 * Get Stationery Stock Register
...
export const createStockItem = async (stockData) => {
  const response = await api.post('/stock-register', stockData);
  return response.data;
};

export const updateStockItem = async (id, stockData) => {
  const response = await api.put(`/stock-register/${id}`, stockData);
  return response.data;
};
...
export const getNextGrNumber = async (branch) => {
  try {
    console.log('📤 Calling getNextGrNumber for branch:', branch);
    const response = await api.get('/dispatch/next-gr', { 
      params: { branch } 
    });
    console.log('📥 getNextGrNumber Response:', response.data);
    
    // ✅ Handle both response formats
    const grNumber = response.data.grNumber || response.data.grBookNumber || null;
    
    if (!grNumber) {
      console.error('❌ No grNumber in response:', response.data);
      throw new Error('GR Number not found in response');
    }
    
    return grNumber;
  } catch (error) {
    console.error('❌ getNextGrNumber Error:', error.response?.data || error.message);
    throw error;
  }
};
...
export const getAllDespatches = async (search = '') => {
  const response = await api.get('/dispatch', { params: { search } });
  return response.data;
};

export const getPendingDespatches = async (branch) => {
...
export const receiveDespatch = async (id, receivedBy) => {

  const response = await api.put(`/dispatch/${id}`, {
    status: 'Received',
...
export const cancelDespatch = async (id) => {
  const response = await api.put(`/dispatch/${id}`, { status: 'Cancelled' });
  return response.data;
};
...
// Get all stock issues with filters
export const getStockIssues = async (filters = {}) => {
  const params = new URLSearchParams();
  Object.keys(filters).forEach(key => {
    if (filters[key] !== undefined && filters[key] !== null && filters[key] !== '') {
      params.append(key, filters[key]);
  ...
  const response = await api.get(`/stock-issue?${params.toString()}`);
  return response.data;
};
...
// Update stock issue
export const updateStockIssue = async (id, data) => {
  const response = await api.put(`/stock-issue/${id}`, data);
  return response.data;
};

// Delete stock issue
export const deleteStockIssue = async (id) => {
  const response = await api.delete(`/stock-issue/${id}`);
  return response.data;
};
