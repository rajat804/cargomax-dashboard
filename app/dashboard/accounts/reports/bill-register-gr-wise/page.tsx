...
export default function BillRegisterGrWise() {
  const [fromDate, setFromDate] = useState<Date>(new Date());
  const [toDate, setToDate] = useState<Date>(new Date());

  const [branchTypeAll, setBranchTypeAll] = useState(true);
  const [customerAll, setCustomerAll] = useState(true);
  ...
  const [mailSubject, setMailSubject] = useState("Bill Register GR Wise Report");
  const [mailBody, setMailBody] = useState("Dear Sir/Madam,\n\nPlease find attached the Bill Register GR Wise Report.\n\nRegards,\nGolden Roadways");

  const handleGenerateReport = () => {
    alert("Report Generated Successfully in Grid View!");
  };
  ...
  const handleSendMail = () => {
    ...
    alert(`Report successfully sent to: ${mailTo}`);
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="border-b pb-4">
        <h1 className="text-2xl font-bold text-primary">BILL REGISTER GR WISE</h1>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <Checkbox 
                checked={customerAll} 
                onCheckedChange={(checked) => setCustomerAll(!!checked)} 
              />
              <Label>ALL</Label>
            </div>
            ...
            {!customerAll && (
              <Select value={selectedCustomer} onValueChange={setSelectedCustomer}>
                <SelectTrigger>
                  <SelectValue placeholder="Select Specific Customer" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="ABC Transport">ABC Transport</SelectItem>
                  <SelectItem value="XYZ Logistics">XYZ Logistics</SelectItem>
                  <SelectItem value="PQR Carriers">PQR Carriers</SelectItem>
                </SelectContent>
              </Select>
            )}
          </div>
